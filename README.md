...
8. [Step 6: Automating the Pipeline](#8-step-6-automating-the-pipeline)
9. [Conclusion](#9-conclusion)
10. [Infrastructure Diagram](#10-infrastructure-diagram)
11. [Work flow Diagram](#11-work-flow-diagram)

---
Note: Some fields in the screenshots (such as IP addresses, usernames, IDs, and credentials) have been intentionally redacted for security and privacy purposes.

### **1. Project Overview**
This document describes the end-to-end process of deploying a two-tier web application (Flask + MySQL) on an AWS EC2 instance. The application is containerized using Docker and orchestrated with Docker Compose. A complete CI/CD pipeline is implemented using Jenkins to automate image builds and application deployment whenever new changes are pushed to the GitHub repository.
...
#### **Dockerfile**
This file defines the runtime environment for the Flask application container.
```dockerfile
...
#### **Jenkinsfile**
This file contains the CI/CD pipeline using Jenkins pipeline-as-code.
```groovy
pipeline {
    agent any
    stages {
...
1.  **Create Credentials for Jenkins to Access GitHub:**
    * From the Jenkins dashboard, select **Manage Jenkins"**.
    * Navigate to **Credentials** → **system** → **add domain**.
    * Create a new domain and add credentials for repository access.
...
2. **Add a webhook in GitHub:**
   ...
   * Save the webhook.
   ...
   Then open Recent Deliveries and confirm a 200 OK response.

---

### **9. Conclusion**
The CI/CD pipeline is now fully operational. Any push to the main branch automatically triggers Jenkins to rebuild the Docker image and redeploy the updated application. This setup provides a reliable, fully automated workflow from development to production.
